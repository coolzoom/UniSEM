...
#Region "vars"
    ...
    ''' <summary>
    ''' what is the keyword for password, passworld will be encrypt
    ''' 密码包含的关键字，密码将会被加密
    ''' </summary>
    Private ReadOnly CONST_KEYWORD_PASSWORD As String = "PASSWORD"

    ''' <summary>
    ''' what is the keyword for password encrypt
    ''' 密码字符加密秘钥
    ''' </summary>
    Private ReadOnly CONST_KEYWORD_PASSWORDENCRYPTKEY As String = "UniSEM"

    ''' <summary>
    ''' how to not decrypt the password if format is wrong
    ''' 如果密码字符错乱则会造成解析失败，将该字符写到密码值可以不做解析并重新设置
    ''' </summary>
    Private ReadOnly CONST_KEYWORD_PASSWORDREMOVE As String = "PASSWORDREMOVE"
            ...
            If Not (Left(line, 1) = CONST_COMMENT_SYMBOL) Then
                ...
                If line.Contains(CONST_VALUE_SEPARATOR) Then
                    ...
                    If arr.Count = 2 Then
                        ...
                        'PASSWORD?
                        If tempKeyword.Contains(CONST_KEYWORD_PASSWORD) And tempValue <> CONST_KEYWORD_PASSWORDREMOVE Then
                            tempValue = Utilities.PortableCryptography.Decrypt(tempValue, CONST_KEYWORD_PASSWORDENCRYPTKEY)
                            If tempValue = "" Then ' if not a valid value then set it to default PASSWORD
                                tempValue = CONST_KEYWORD_PASSWORD
                            End If
                        End If
...
#Region "private functions"
    ...
    Public Sub SaveConfigFromDictToFile()
        ...
        For Each subKey In dictConfigKeyWordAsKeyDetailAsValue
            ...
            End If
            If subKey.Value.ConfigKeyWord.Contains(CONST_KEYWORD_PASSWORD) Then
                sw.WriteLine(String.Format("{0} {1} {2}", ExtendStringLength(subKey.Value.ConfigKeyWord),
                                                            CONST_VALUE_SEPARATOR,
                                                            Utilities.PortableCryptography.Encrypt(subKey.Value.ConfigVALUE, CONST_KEYWORD_PASSWORDENCRYPTKEY)
                                                            ))
            Else
    ...
    Public Sub SaveConfigFromDataTableToFile()
        'Method1, replace parameter, 方法1,只替换找到的内容
        ''to contain the temperory config content
        'Dim tempContent As String = ""
        ...
        For Each subrow As DataRow In ResultTable.Rows
            If subrow(2).ToString <> "" Then
                sw.WriteLine("") 'always add empty line before description 描述前始终添加空行
                sw.WriteLine(subrow(2).ToString) '描述写在前面
            End If
            If subrow(0).ToString.Contains(CONST_KEYWORD_PASSWORD) Then
                sw.WriteLine(String.Format("{0} {1} {2}", ExtendStringLength(subrow(0).ToString),
                                                             CONST_VALUE_SEPARATOR,
                                                            Utilities.PortableCryptography.Encrypt(subrow(1).ToString, CONST_KEYWORD_PASSWORDENCRYPTKEY)
                                                            ))
            Else
                sw.WriteLine(String.Format("{0} {1} {2}", ExtendStringLength(subrow(0).ToString), CONST_VALUE_SEPARATOR, subrow(1).ToString))
            End If
